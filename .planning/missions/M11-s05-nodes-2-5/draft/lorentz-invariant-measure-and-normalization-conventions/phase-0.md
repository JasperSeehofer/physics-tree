...
**Now the part that makes it a node rather than a fact.** Once you know the invariant combination, you have a *choice* about where to put it, and the choice is genuinely free. The $\sqrt{2E}$ can live:

- **in the measure**, leaving $\lvert\mathbf{k}\rangle = a^{\dagger}_{\mathbf{k}}\lvert0\rangle$ — this branch, following Peskin;
- **in the state**, $\lvert\mathbf{k}\rangle = \sqrt{2E_{\mathbf{k}}}\,a^{\dagger}_{\mathbf{k}}\lvert0\rangle$ — the relativistically normalized convention;
- **in the operator**, absorbing it into $a_{\mathbf{k}}$ itself — Srednicki.

All three are used in print, all three are correct, and **none of them is more right than the others**. What is not free is consistency: fix any one and the other two are determined, by a single identity this node derives and Phase 3 turns into a fifteen-second check.
...
This node's version of that failure is worse, because it happens further from where you would look. Take the amplitude from a source that normalizes states relativistically, insert it into the phase-space integral of a source that does not, and you get a cross section wrong by $2E$ per external leg. In node 24's $e^{+}e^{-}\to\mu^{+}\mu^{-}$ that is a factor of $(2E)^{4}$ hiding in a number you can look up — and, since it is dimensionless-looking and energy-dependent, it will not even fail a dimensional check cleanly.
