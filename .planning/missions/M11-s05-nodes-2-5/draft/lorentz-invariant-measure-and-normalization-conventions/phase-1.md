...
**Part C5 — the cost, and the reason this node has no correctness gate.** Each external leg contributes one power of $\sqrt{2E}$ per amplitude and hence one power of $2E$ in $\lvert\mathcal{M}\rvert^{2}$; a mismatch on $n$ external legs is a factor $\prod_{i}(2E_{i})^{\pm1}$. It is **energy-dependent**, so it is not a constant you could absorb; and it is **dimensionful**, carrying mass dimension $n$, so the cross section comes out with the wrong dimension entirely. **A dimensional check on the final answer therefore does catch it** — which is exactly the defence node 1 named as the cheapest available. What no check catches is *where* it entered: every symbol on the page is the symbol its source wrote, every line is internally consistent, and nothing is misplaced. The error lives in the join. **Detecting it is easy; locating it is not** — which is why this node's answer is an identity you run on the join itself, and why there is no single wrong probe answer to gate on.
