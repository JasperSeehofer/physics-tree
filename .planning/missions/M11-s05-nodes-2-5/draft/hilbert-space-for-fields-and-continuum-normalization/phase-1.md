...
**Part B1–B2 — the contradiction, correctly stated.** $\langle x\lvert x\rangle = \delta(0)$, not a number; $[\delta(x)]$ is an inverse length, and in three dimensions $\delta^{3}(0) = V/(2\pi)^{3}$, a *volume*. Sharply: **$\mathbb{1} = \int dx\,\lvert x\rangle\langle x\rvert$ is an identity between well-defined operators on $\mathcal{H}$, built entirely out of objects that are not in $\mathcal{H}$.**

The resolution is not that one side is approximate. It is that $\lvert x\rangle\langle x\rvert$ never has to be evaluated alone — it sits inside an integral between well-behaved states, and there it is perfectly finite. The structure making that precise is the **rigged Hilbert space** of Phase 2's Abstract Stage.
