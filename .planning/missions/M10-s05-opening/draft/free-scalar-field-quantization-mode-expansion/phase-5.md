...
Copper-like numbers: $M = 63.5\ \mathrm{u} = 1.055\times10^{-25}\ \mathrm{kg}$, $a = 2.55\ \mathrm{\AA}$, longitudinal sound speed $v = 3810\ \mathrm{m/s}$.
...
Radiation domination has $\alpha\propto\eta$, so $\alpha'' = 0$. What is the mode equation then, and what does that say about a massless scalar in a radiation-dominated universe? (The property has a name.)
