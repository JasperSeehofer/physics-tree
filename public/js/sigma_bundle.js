...
var DEFAULT_FIT_VIEWPORT_TO_NODES_OPTIONS = {
  animate: true
};
function getCameraStateToFitViewportToNodes(sigma, nodes) {
  if (!nodes.length) throw new Error("getCameraStateToFitViewportToNodes: There should be at least one node.");
  var groupMinX = Infinity;
  var groupMaxX = -Infinity;
  var groupMinY = Infinity;
  var groupMaxY = -Infinity;
  var groupFramedMinX = Infinity;
  var groupFramedMaxX = -Infinity;
  var groupFramedMinY = Infinity;
  var groupFramedMaxY = -Infinity;
  var graph = sigma.getGraph();
  nodes.forEach(function(node) {
    var data = sigma.getNodeDisplayData(node);
    if (!data) throw new Error("getCameraStateToFitViewportToNodes: Node ".concat(node, " not found in sigma's graph."));
    var _graph$getNodeAttribu = graph.getNodeAttributes(node), x2 = _graph$getNodeAttribu.x, y2 = _graph$getNodeAttribu.y;
    var framedX = data.x, framedY = data.y;
    groupMinX = Math.min(groupMinX, x2);
    groupMaxX = Math.max(groupMaxX, x2);
    groupMinY = Math.min(groupMinY, y2);
    groupMaxY = Math.max(groupMaxY, y2);
  ...
  var _ref = sigma.getCustomBBox() || sigma.getBBox(), x = _ref.x, y = _ref.y;
  var graphWidth = x[1] - x[0] || 1;
  var graphHeight = y[1] - y[0] || 1;
  var groupCenterX = (groupFramedMinX + groupFramedMaxX) / 2;
  var groupCenterY = (groupFramedMinY + groupFramedMaxY) / 2;
  var groupWidth = groupMaxX - groupMinX || graphWidth;
  var groupHeight = groupMaxY - groupMinY || graphHeight;
  var _sigma$getDimensions = sigma.getDimensions(), width = _sigma$getDimensions.width, height = _sigma$getDimensions.height;
  var correction = getCorrectionRatio({
    width,
    height
  }, {
    width: graphWidth,
  ...
  var ratio = (groupHeight / groupWidth < height / width ? groupWidth : groupHeight) / Math.max(graphWidth, graphHeight) * correction;
  var camera = sigma.getCamera();
  return _objectSpread22(_objectSpread22({}, camera.getState()), {}, {
    angle: 0,
    x: groupCenterX,
    y: groupCenterY,
    ratio
...
function fitViewportToNodes(_x, _x2) {
  return _fitViewportToNodes.apply(this, arguments);
}
function _fitViewportToNodes() {
  _fitViewportToNodes = _asyncToGenerator(/* @__PURE__ */ _regeneratorRuntime().mark(function _callee(sigma, nodes) {
    var opts, _DEFAULT_FIT_VIEWPORT, animate, camera, newCameraState, _args = arguments;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          opts = _args.length > 2 && _args[2] !== void 0 ? _args[2] : {};
          _DEFAULT_FIT_VIEWPORT = _objectSpread22(_objectSpread22({}, DEFAULT_FIT_VIEWPORT_TO_NODES_OPTIONS), opts), animate = _DEFAULT_FIT_VIEWPORT.animate;
          camera = sigma.getCamera();
          newCameraState = getCameraStateToFitViewportToNodes(sigma, nodes);
          if (!animate) {
            _context.next = 9;
            break;
          }
          _context.next = 7;
          return camera.animate(newCameraState);
        case 7:
          _context.next = 10;
          break;
        case 9:
...
var prereqChainSet = /* @__PURE__ */ new Set();
var userProgressMap = {};
var overdueMap = {};
var COLORS = {
  void: "#0d0f14",
...
var EDGE_TYPE_STYLES = {
  ...
  AppliesTo: { color: COLORS.barkLight, size: 1, lineStyle: "dotted" },
  MathematicalFoundation: { color: COLORS.nebulaPurple, size: 2, lineStyle: "double" }
};
function drawEdgeOverlay() {
  ...
  graphInstance.forEachEdge((edge, attrs, source, target) => {
    ...
    if (style.lineStyle === "solid") return;
    ...
    if (selectedNodeId !== null) {
      const isHighlighted = (prereqChainSet.has(source) || source === selectedNodeId) && (prereqChainSet.has(target) || target === selectedNodeId);
      if (isHighlighted) {
...
function botanicalNodeReducer(node, data) {
  ...
  if (Object.keys(userProgressMap).length > 0) {
    ...
    if (nodeXp !== void 0) {
      if (nodeXp >= 300) {
        res.label = `${data.label} \u2014 Gold \xB7 Mastered`;
      } else if (nodeXp >= 150) {
        res.label = `${data.label} \u2014 Silver \xB7 ${nodeXp} XP`;
      } else if (nodeXp >= 50) {
        res.label = `${data.label} \u2014 Bronze \xB7 ${nodeXp} XP`;
      } else if (nodeXp > 0) {
  ...
  if (daysOverdue !== void 0 && daysOverdue >= 1) {
...
function initSigma(container, onNodeClick, onNodeEnter, onNodeLeave) {
...
function updateUserProgress(progressJson) {
  userProgressMap = progressJson ? JSON.parse(progressJson) : {};
  if (sigmaInstance) sigmaInstance.refresh();
}
function updateOverdueMap(overdueJson) {
  overdueMap = overdueJson ? JSON.parse(overdueJson) : {};
  if (sigmaInstance) sigmaInstance.refresh();
}
function loadGraphData(nodesJson, edgesJson) {
  if (!graphInstance) return;
  const nodes = JSON.parse(nodesJson);
  const edges = JSON.parse(edgesJson);
  const depthYMap = { root: 0, trunk: 0.3, branch: 0.6, leaf: 1 };
  const branchGroups = {};
  nodes.forEach((node) => {
  ...
  fa2Worker = new import_worker.default(graphInstance, {
    settings: {
      gravity: 1,
      scalingRatio: 10,
...
function highlightPrereqChain(nodeId, prereqIdsJson) {
  selectedNodeId = nodeId || null;
  prereqChainSet = /* @__PURE__ */ new Set();
